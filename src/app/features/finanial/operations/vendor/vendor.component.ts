...
import {
  vendorHeaderDto,
  filterVendorHeaderDto,
  filtervendorHeaderByIDDto,
  loadVendorNameDto,
} from '../../../../core/dtos/FinancialDtos/OperationDtos/vendor.models';
...
export class VendorComponent implements OnInit {
    ...
    this.vendorSearchInput$
      ...
      .subscribe((searchTerm: string) => {
        if (this.searchParams.entityId) {
          ...
          this.fetchVendorNameSelect2(
            this.searchParams.entityId,
            this.vendorsearchParams.searchValue
          );
        }
      });
  ...
  getFormDatabyId(vendoR_ID: any, entitY_ID: any): void {
    ...
    forkJoin({
      vendorHeaderData: this.vendorService.getDetailById(params) as Observable<
        vendorHeaderDto | vendorHeaderDto[]
      >,
    })
      .pipe(takeUntil(this.destroy$))
      .subscribe({
  ...
  public buildColumnDefs(): void {
    this.columnDefs = [
      {
        headerName: this.translate.instant('vendorResourceName.vendorNo'),
        field: 'vendoR_NUMBER',
        width: 200,
      },
      {
        headerName: this.translate.instant('vendorResourceName.vendorName'),
        field: 'vendoR_NAME',
        width: 200,
      },
      {
        headerName: this.translate.instant('vendorResourceName.status'),
        field: 'statuS_DESC',
        width: 200,
      },
      {
        headerName: this.translate.instant('vendorResourceName.category'),
        field: 'categorY_DESC',
        width: 200,
      },
      {
        headerName: this.translate.instant('vendorResourceName.address'),
        field: 'address',
        width: 200,
      },
    ];
                  ...
                  fields: [
                    {
                      label: this.translate.instant(
                        'vendorResourceName.entityId'
                      ),
                      value: this.searchParams.entityIdstr,
                    },
                    {
                      label: this.translate.instant(
                        'vendorResourceName.vendorName'
                      ),
                      value: this.searchParams.VendorName,
                    },
                    {
                      label: this.translate.instant(
                        'vendorResourceName.status'
                      ),
                      value: this.searchParams.status,
                    },
                  ],
                  columns: [
                    ...
                    {
                      label: this.translate.instant(
                        'vendorResourceName.category'
                      ),
                      key: 'categorY_DESC',
                    },
                    {
                      label: this.translate.instant(
                        'vendorResourceName.address'
                      ),
                      key: 'address',
                    },
                  ],
