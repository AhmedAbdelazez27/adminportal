...
import { AttachmentBase64Dto, AttachmentDto } from '../../../core/dtos/attachments/attachment.dto';
...
export class InitiativeComponentComponent implements OnInit, OnDestroy {
  ...
  private initializeColumnDefinitions(): void {
    this.columnDefs = [
      ...
      {
        field: 'nameAr',
        headerName: this.translate.instant('INITIATIVE.NAME_AR') || 'Initiative Name (Arabic)',
        sortable: true,
        filter: true,
        width: 180,
      },
      {
        field: 'nameEn',
        headerName: this.translate.instant('INITIATIVE.NAME_EN') || 'Initiative Name (English)',
        sortable: true,
        filter: true,
        width: 180,
      },
      {
        field: 'targetGroup',
        headerName: this.translate.instant('INITIATIVE.TARGET_GROUP') || 'Target Group',
        sortable: true,
        filter: true,
        width: 150,
      },
      {
        ...
        headerName: this.translate.instant('INITIATIVE.DATE') || 'Initiative Date',
        sortable: true,
        filter: true,
        width: 130,
      ...
      },
      {
        ...
        cellRenderer: (params: any) => {
          return params.value
            ? '<span class="status-active">' + (this.translate.instant('COMMON.ACTIVE') || 'Active') + '</span>'
            : '<span class="status-inactive">' + (this.translate.instant('COMMON.INACTIVE') || 'Inactive') + '</span>';
        },
      },
    ];
    ...
    this.detailsColumnDefs = [
      ...
      },
      {
        ...
        headerName: this.translate.instant('INITIATIVE.LOCATION_NAME_AR') || 'Location Name (Arabic)',
        sortable: true,
        filter: true,
        width: 180,
      },
      {
        ...
        headerName: this.translate.instant('INITIATIVE.LOCATION_NAME_EN') || 'Location Name (English)',
        sortable: true,
        filter: true,
        width: 180,
      },
      {
        ...
        headerName: this.translate.instant('INITIATIVE.COORDINATES') || 'Coordinates',
        sortable: true,
        filter: true,
        width: 150,
      },
    ...
    this.rowActions = [
      { label: this.translate.instant('COMMON.VIEW') || 'View', action: 'view', icon: 'icon-frame-view' },
      { label: this.translate.instant('COMMON.EDIT') || 'Edit', action: 'edit', icon: 'icon-frame-edit' },
      { label: this.translate.instant('COMMON.DELETE') || 'Delete', action: 'delete', icon: 'icon-frame-delete' },
    ];
  ...
  loadInitiatives(): void {
    ...
    this.initiativeService.getAllAsync(params).subscribe({
      ...
      error: (error: any) => {
        this.toastr.error(this.translate.instant('INITIATIVE.MESSAGES.ERROR_LOADING_INITIATIVES') || 'Error loading initiatives');
        this.isLoading = false;
        this.spinnerService.hide();
      },
    });
  ...
  private loadAttachmentsConfig(): void {
    this.attachmentService
      ...
      .subscribe({
        ...
        error: (error) => {
          this.toastr.error(this.translate.instant('INITIATIVE.MESSAGES.ERROR_LOADING_ATTACHMENT_CONFIG') || 'Error loading attachment configuration');
  ...
  openEditModal(initiative: InitiativeDto): void {
    this.initiativeService.getById(initiative.id).subscribe({
      ...
      error: async (error) => {
        this.toastr.error(this.translate.instant('INITIATIVE.MESSAGES.ERROR_LOADING_INITIATIVE_DETAILS') || 'Error loading initiative details');
  ...
  private convertToMainApplyServiceAttachmentDto(attachment: AttachmentDto): MainApplyServiceAttachmentDto {
    return {
      id: attachment.id,
      ...
      lastModifiedstr: attachment.lastModified ? attachment.lastModified.toISOString() : null,
      attConfigID: attachment.attConfigID,
      attachmentBinary: null
    };
  }

  private convertToGalleryAttachmentDto(attachment: MainApplyServiceAttachmentDto): AttachmentDto {
    return {
  ...
  private populateForm(initiative: InitiativeDto): void {
    this.initiativeForm.patchValue({
      ...
      initiativeDate: initiative.initiativeDate ? new Date(initiative.initiativeDate).toISOString().split('T')[0] : '',
    ...
    if (initiative.initiativeDetails && initiative.initiativeDetails.length > 0) {
      initiative.initiativeDetails.forEach(detail => {
        const detailForm = this.fb.group({
          locationNameAr: [detail.locationNameAr, [Validators.required, Validators.minLength(2)]],
          locationNameEn: [detail.locationNameEn, [Validators.required, Validators.minLength(2)]],
          locationCoordinates: [detail.locationCoordinates, [Validators.required]],
  ...
  closeModal(): void {
    ...
    if (this.initiativeModalInstance) {
      ...
      if (initiativeModal) {
        const modal = (window as any).bootstrap.Modal.getInstance(initiativeModal);
        if (modal) {
          modal.hide();
        }
      }
  ...
  removeFile(): void {
    ...
    this.fileValidationSuccess = false;
    this.uploadProgress = 0;
    const fileInput = document.getElementById('initiativeImage') as HTMLInputElement;
    if (fileInput) {
      fileInput.value = '';
    }
  }
  ...
  confirmDeleteAttachment(): void {
    ...
    this.spinnerService.show();
    this.attachmentService.deleteAsync(this.selectedAttachmentToDelete.id).subscribe({
      next: () => {
        this.toastr.success(this.translate.instant('INITIATIVE.MESSAGES.ATTACHMENT_DELETED') || 'Attachment deleted successfully');
        this.existingAttachment = null;
        this.existingImageUrl = null;
        this.selectedAttachmentToDelete = null;
      ...
      error: (error) => {
        this.toastr.error(this.translate.instant('INITIATIVE.MESSAGES.ERROR_DELETING_ATTACHMENT') || 'Error deleting attachment');
        this.spinnerService.hide();
      },
    });
  }
  ...
  private validateAndSetFile(file: File): void {
    ...
    if (!allowedTypes.includes(file.type)) {
      this.fileValidationErrors.push(this.translate.instant('INITIATIVE.FILE_UPLOAD.INVALID_FILE_TYPE') || 'Invalid file type. Only PDF, JPG, and PNG files are allowed.');
      return;
    }

    if (file.size > maxSize) {
      this.fileValidationErrors.push(this.translate.instant('INITIATIVE.FILE_UPLOAD.FILE_SIZE_ERROR') || 'File size must be less than 2MB.');
      return;
    }
    ...
    if (this.mode === 'edit' && this.existingImageUrl) {
      this.toastr.info(this.translate.instant('INITIATIVE.FILE_UPLOAD.NEW_IMAGE_SELECTED') || 'New image selected. This will replace the current image when you save.');
    }
  }
  ...
  onImageError(event: any): void {
    ...
    this.toastr.error(this.translate.instant('INITIATIVE.FILE_UPLOAD.IMAGE_LOAD_ERROR') || 'Failed to load image. The image file may not exist or the server may be unavailable.', 'Image Load Error');
  }
  ...
  async submit(): Promise<void> {
    ...
    if (initiativeImageConfig) {
      if (this.mode === 'add' && !hasNewFile) {
        this.toastr.error(this.translate.instant('INITIATIVE.FILE_UPLOAD.IMAGE_REQUIRED') || 'Initiative image is required');
        return;
      }
    ...
    try {
      ...
      if (this.selectedFile && initiativeImageConfig) {
        ...
        attachmentDto = {
          ...
          masterId: this.mode === 'edit' && this.editingInitiativeId ? this.editingInitiativeId : 0,
      ...
      if (this.mode === 'add') {
        ...
        this.initiativeService.createAsync(createDto).subscribe({
          next: () => {
            this.toastr.success(this.translate.instant('INITIATIVE.MESSAGES.INITIATIVE_CREATED') || 'Initiative created successfully');
            this.closeModal();
            this.loadInitiatives();
            this.spinnerService.hide();
          },
          error: (error) => {
            this.toastr.error(this.translate.instant('INITIATIVE.MESSAGES.ERROR_CREATING_INITIATIVE') || 'Error creating initiative');
            this.spinnerService.hide();
          },
        });
        ...
        this.initiativeService.updateAsync(updateDto).subscribe({
          next: () => {
            this.toastr.success(this.translate.instant('INITIATIVE.MESSAGES.INITIATIVE_UPDATED') || 'Initiative updated successfully');
          ...
          error: (error) => {
            this.toastr.error(this.translate.instant('INITIATIVE.MESSAGES.ERROR_UPDATING_INITIATIVE') || 'Error updating initiative');
            this.spinnerService.hide();
          },
        });
      }
    } catch (error) {
      this.toastr.error(this.translate.instant('INITIATIVE.MESSAGES.ERROR_PROCESSING_FILE') || 'Error processing file');
      this.spinnerService.hide();
    }
  }
  ...
  private updateAttachmentSeparately(attachmentDto: AttachmentBase64Dto): void {
    ...
    this.attachmentService.updateAsync(updateAttachmentDto).subscribe({
      next: (response) => {
        this.toastr.success(this.translate.instant('INITIATIVE.MESSAGES.IMAGE_UPDATED') || 'Image updated successfully');
        this.refreshImageAfterUpdate();
        this.closeModal();
        this.loadInitiatives();
        this.spinnerService.hide();
      ...
      error: (error) => {
        this.toastr.error(this.translate.instant('INITIATIVE.MESSAGES.ERROR_UPDATING_IMAGE') || 'Error updating image');
        this.spinnerService.hide();
      },
    });
  }
  ...
  private createAttachmentSeparately(attachmentDto: AttachmentBase64Dto): void {
    this.attachmentService.saveAttachmentFileBase64(attachmentDto).subscribe({
      next: (response) => {
        this.toastr.success(this.translate.instant('INITIATIVE.MESSAGES.IMAGE_UPLOADED') || 'Image uploaded successfully');
        this.refreshImageAfterUpdate();
        this.closeModal();
        this.loadInitiatives();
        this.spinnerService.hide();
      ...
      error: (error) => {
        this.toastr.error(this.translate.instant('INITIATIVE.MESSAGES.ERROR_UPLOADING_IMAGE') || 'Error uploading image');
        this.spinnerService.hide();
      },
    });
  }
  ...
  private refreshImageAfterUpdate(): void {
    if (this.editingInitiativeId) {
      ...
      this.attachmentService
        .getListByMasterId(this.editingInitiativeId, AttachmentsConfigType.Initiative)
        .subscribe({
  ...
  private getInitiativeImageConfig(): any {
    return this.attachmentConfigs.find(
      (config) => config.attachmentsConfigType === AttachmentsConfigType.Initiative
    );
  }
...
deleteInitiative(): void {
  ...
  getFieldError(fieldName: string): string {
    const field = this.initiativeForm.get(fieldName);
    if (field?.errors) {
      if (field.errors['required']) return this.translate.instant('INITIATIVE.MESSAGES.FIELD_REQUIRED') || 'This field is required';
  ...
  saveInitiativeDetail(): void {
    ...
    if (this.editingDetailIndex !== null) {
      ...
      this.initiativeDetailsFormArray.at(this.editingDetailIndex).patchValue(detailValue);
      this.editingDetailIndex = null;
    } else {
      // Add new detail
      const detailForm = this.fb.group({
        locationNameAr: [detailValue.locationNameAr, [Validators.required, Validators.minLength(2)]],
        locationNameEn: [detailValue.locationNameEn, [Validators.required, Validators.minLength(2)]],
        locationCoordinates: [detailValue.locationCoordinates, [Validators.required]],
  ...
  initDetailsMap(): void {
    try {
      if (typeof L === 'undefined') {
        this.toastr.error(this.translate.instant('INITIATIVE.MAP.MAP_LIBRARY_ERROR') || 'Map library not loaded. Please refresh the page.');
        return;
      }
      ...
      setTimeout(() => {
        ...
        if (this.selectedCoordinates) {
          this.addMarkerToDetailsMap(this.selectedCoordinates.lat, this.selectedCoordinates.lng);
          this.detailsMap.setView([this.selectedCoordinates.lat, this.selectedCoordinates.lng], 15);
        }
  ...
  private addMarkerToDetailsMap(lat: number, lng: number): void {
    ...
    try {
      this.detailsMarker = L.marker([lat, lng], { 
        ...
        title: this.detailsMode === 'view' ? this.translate.instant('INITIATIVE.MAP.LOCATION_MARKER') || 'Location marker' : this.translate.instant('INITIATIVE.MAP.DRAG_MARKER') || 'Click and drag to move the marker'
      ...
      const popupContent = `
        <div style="font-family: inherit; font-size: 14px;">
          <strong>📍 ${this.detailsMode === 'view' ? this.translate.instant('INITIATIVE.MAP.LOCATION_MARKER') || 'Location' : this.translate.instant('INITIATIVE.MAP.SELECTED_LOCATION') || 'Selected Location'}</strong><br/>
          <strong>Lat:</strong> ${lat.toFixed(6)}<br/>
          <strong>Lng:</strong> ${lng.toFixed(6)}<br/>
          ${this.detailsMode !== 'view' ? '<small style="color: #666;">Drag marker to adjust position</small>' : ''}
        </div>
      `;
      ...
      if (this.detailsMode !== 'view') {
        ...
        this.toastr.success(this.translate.instant('INITIATIVE.MAP.LOCATION_SELECTED') || 'Location selected! You can drag the marker to adjust the position.');
      }
  ...
  onManualCoordinateInput(): void {
    ...
    try {
      ...
      if (!isNaN(lat) && !isNaN(lng) && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180) {
        ...
        this.getAddressFromCoordinates(lat, lng);
        this.toastr.success(this.translate.instant('INITIATIVE.MAP.COORDINATES_SET') || 'Coordinates set successfully');
      } else {
        this.toastr.error(this.translate.instant('INITIATIVE.MAP.INVALID_COORDINATES') || 'Invalid coordinates. Please use format: latitude/longitude (e.g., 25.2048/55.2708)');
      }
    } catch (error) {
      this.toastr.error(this.translate.instant('INITIATIVE.MAP.INVALID_COORDINATES_FORMAT') || 'Invalid coordinates format. Please use: latitude/longitude');
    }
  }
  ...
  getAddressFromCoordinates(lat: number, lng: number): Promise<void> {
    ...
    return Promise.race([fetch(englishUrl), timeoutPromise])
      ...
      .then((englishData) => {
           ...
           this.addressFromCoordinates = typeof englishData.display_name === 'string' ? englishData.display_name : JSON.stringify(englishData.display_name);
      ...
      .catch((error) => {
        this.isMapLoading = false;
        this.toastr.error(this.translate.instant('INITIATIVE.MAP.ERROR_GETTING_ADDRESS') || 'Error getting address from coordinates');
  ...
  getCoordinatesFromDetailAddress(): void {
    ...
    if (!locationNameEn && !locationNameAr) {
      this.toastr.warning(this.translate.instant('INITIATIVE.MAP.ENTER_LOCATION_FIRST') || 'Please enter a location name first (English or Arabic)');
      return;
    }
    ...
    Promise.race([fetch(url), timeoutPromise])
      ...
      .then((data) => {
        ...
        if (data?.length > 0) {
          ...
          this.toastr.success(this.translate.instant('INITIATIVE.MAP.COORDINATES_FOUND') || 'Coordinates found and set on map');
        } else {
          this.toastr.warning(this.translate.instant('INITIATIVE.MAP.NO_COORDINATES_FOUND') || 'No coordinates found for this location');
        }
      })
      .catch((error) => {
        this.spinnerService.hide();
        this.isMapLoading = false;
        this.toastr.error(this.translate.instant('INITIATIVE.MAP.ERROR_GETTING_COORDINATES') || 'Error getting coordinates from location name');
      });
  }
  ...
  useCurrentLocation(): void {
    if (!navigator.geolocation) {
      this.toastr.error(this.translate.instant('INITIATIVE.MAP.GEOLOCATION_NOT_SUPPORTED') || 'Geolocation is not supported by this browser');
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        this.toastr.success(this.translate.instant('INITIATIVE.MAP.CURRENT_LOCATION_SET') || 'Current location set successfully');
      },
      (error) => {
        this.spinnerService.hide();
        this.isMapLoading = false;
        let errorMessage = this.translate.instant('INITIATIVE.MAP.ERROR_GETTING_LOCATION') || 'Error getting current location';
        
        switch (error.code) {
          case error.PERMISSION_DENIED:
            errorMessage = this.translate.instant('INITIATIVE.MAP.LOCATION_PERMISSION_DENIED') || 'Location permission denied';
            break;
          case error.POSITION_UNAVAILABLE:
            errorMessage = this.translate.instant('INITIATIVE.MAP.LOCATION_UNAVAILABLE') || 'Location information unavailable';
            break;
          case error.TIMEOUT:
            errorMessage = this.translate.instant('INITIATIVE.MAP.LOCATION_TIMEOUT') || 'Location request timed out';
            break;
        }
  ...
  selectDetailToDelete(detail: CreateInitiativeDetailsDto, index: number): void {
  ...
  deleteDetail(): void {
    if (this.selectedDetailIndex >= 0) {
      ...
      this.toastr.success(this.translate.instant('INITIATIVE.MESSAGES.DETAIL_DELETED') || 'Initiative detail deleted successfully');
    }
  }
