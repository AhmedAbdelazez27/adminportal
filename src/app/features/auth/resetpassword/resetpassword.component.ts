...
export class ResetpasswordComponent {
  ...
  submit(): void {
    ...
    this.authService.resetPassword(data).subscribe({
      next: (res) => {
        this.toastr.success(this.translate.instant('PASSWORD_RESET.SUCCESS'), this.translate.instant('TOAST.TITLE.SUCCESS'));
        this.router.navigate(['/login']);
        this.spinnerService.hide();
      },
      error: (error) => {
        this.toastr.error(this.translate.instant('PASSWORD_RESET.FAILED'), this.translate.instant('TOAST.TITLE.ERROR'));
