...
import { Pagination, SelectdropdownResultResults, FndLookUpValuesSelect2RequestDto, SelectdropdownResult, reportPrintConfig, Select2RequestDto, FndLookUpValuesSelect2RequestbyIdDto } from '../../../../core/dtos/FndLookUpValuesdtos/FndLookUpValues.dto';
...
@Component({
  selector: 'app-ordersListCityRpt',
  standalone: true,
  imports: [CommonModule, FormsModule, TranslateModule, NgSelectComponent, GenericDataTableComponent],
  templateUrl: './ordersListCityRpt.component.html',
  styleUrls: ['./ordersListCityRpt.component.scss']
})

export class ordersListCityRptComponent {
  ...
  fetchcaseIdSelect2(): void {
    ...
    this.searchSelect2ByIdParams.skip = this.caseIdsearchParams.skip;
    this.searchSelect2ByIdParams.take = this.caseIdsearchParams.take;

    this.Select2Service.getSPCasesEntitySelect2(this.searchSelect2ByIdParams)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (response: SelectdropdownResult) => {
          const newItems = response?.results || [];
  ...
  oncaseIdSelect2Change(selected: any): void {
    if (selected) {
      this.searchParams.caseId = selected.id;
      this.searchParams.caseName = selected.text;
    } else {
  ...
  public buildColumnDefs(): void {
    this.translate.get([
      ...
      'SocialCaseReportsResourceName.citY_DESC'
    ]).subscribe(translations => {
      this.columnDefs = [
        ...
        { headerName: translations['SocialCaseReportsResourceName.citY_DESC'], field: 'citY_DESC', width: 200 },
      ];
    });
  }
            ...
            .subscribe({
              next: (response: any) => {
                ...
                const reportConfig: reportPrintConfig = {
                  title: this.translate.instant('SocialCaseReportsResourceName.ordersListCityRptTitle'),
                  reportTitle: this.translate.instant('SocialCaseReportsResourceName.ordersListCityRptTitle'),
                  fileName: `${this.translate.instant('SocialCaseReportsResourceName.ordersListCityRptTitle')}_${new Date().toISOString().slice(0, 10)}.xlsx`,
                  fields: [
                  ...
                  columns: [
                    ...
                    { label: this.translate.instant('SocialCaseReportsResourceName.citY_DESC'), key: 'citY_DESC' },
                  ],
          ...
          this.socialCasesReportsService.getordersListRptData({ ...cleanedFilters, skip: 0, take: totalCount })
            ...
            .subscribe({
              next: (response: any) => {
                ...
                const reportConfig: reportPrintConfig = {
                  title: this.translate.instant('SocialCaseReportsResourceName.ordersListCityRpt_Title'),
                  reportTitle: this.translate.instant('SocialCaseReportsResourceName.ordersListCityRpt_Title'),
                  fields: [
                    { label: this.translate.instant('SocialCaseReportsResourceName.entityId'), value: this.searchParams.entityName },
                    { label: this.translate.instant('SocialCaseReportsResourceName.type'), value: this.searchParams.caseName },
                    { label: this.translate.instant('SocialCaseReportsResourceName.collectorName'), value: this.searchParams.brancheName },
                  ...
                  columns: [
                    ...
                    // { label: this.translate.instant('SocialCaseReportsResourceName.branchE_CODE'), key: 'branchE_CODE' },
                    { label: this.translate.instant('SocialCaseReportsResourceName.statuS_DESC'), key: 'statuscodE_DESC' },
                    { label: this.translate.instant('SocialCaseReportsResourceName.citY_DESC'), key: 'citY_DESC' },
                    // { label: this.translate.instant('SocialCaseReportsResourceName.citY_ID'), key: 'citY_ID' },
                  ],
                  data,
                  totalLabel: this.translate.instant('Common.Total'),
                  totalKeys: ['toT_INCOME', 'toT_DUTIES']
