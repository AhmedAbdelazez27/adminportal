...
import { casesEntitiesRptInputDto } from '../../../../core/dtos/socialcases/reports/socialcasesReporstInput.dto';
import { casesEntitiesRptOutputDto } from '../../../../core/dtos/socialcases/reports/socialcasesReporstOutput.dto';
import { SocialCasesReportsService } from '../../../../core/services/socialcases/reports/socialcasesreports.service';

@Component({
  selector: 'app-casesEntitiesRpt',
  standalone: true,
  imports: [CommonModule, FormsModule, TranslateModule, NgSelectComponent, GenericDataTableComponent],
  templateUrl: './casesEntitiesRpt.component.html',
  styleUrls: ['./casesEntitiesRpt.component.scss']
})

export class casesEntitiesRptComponent {
  ...
  searchParams = new casesEntitiesRptInputDto();
  getAllDataForReports: casesEntitiesRptOutputDto[] = [];
  ...
  getLoadDataGrid(event: { pageNumber: number; pageSize: number }): void {
    ...
    this.socialCasesReportsService.getcasesEntitiesRptData(this.searchParams)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (response: any) => {
  ...
  clear(): void {
    this.searchParams = new casesEntitiesRptInputDto();
  ...
  printExcel(): void {
    ...
    this.socialCasesReportsService.getcasesEntitiesRptData(cleanedFilters)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (initialResponse: any) => {
          const totalCount = initialResponse?.totalCount || initialResponse?.data?.length || 0;

          this.socialCasesReportsService.getcasesEntitiesRptData({ ...cleanedFilters, skip: 0, take: totalCount })
            .pipe(takeUntil(this.destroy$))
            .subscribe({
              next: (response: any) => {
                ...
                const reportConfig: reportPrintConfig = {
                  title: this.translate.instant('SocialCaseReportsResourceName.casesEntitiesRptTitle'),
                  reportTitle: this.translate.instant('SocialCaseReportsResourceName.casesEntitiesRptTitle'),
                  fileName: `${this.translate.instant('SocialCaseReportsResourceName.casesEntitiesRptTitle')}_${new Date().toISOString().slice(0, 10)}.xlsx`,
                  fields: [
                  ...
                  columns: [
                    ...
                    { label: this.translate.instant('SocialCaseReportsResourceName.entitY_NAME'), key: 'entitY_NAME' },
          ...
          this.socialCasesReportsService.getcasesEntitiesRptData({ ...cleanedFilters, skip: 0, take: totalCount })
            ...
            .subscribe({
              next: (response: any) => {
                ...
                const reportConfig: reportPrintConfig = {
                  title: this.translate.instant('SocialCaseReportsResourceName.casesEntitiesRpt_Title'),
                  reportTitle: this.translate.instant('SocialCaseReportsResourceName.casesEntitiesRpt_Title'),
                  fields: [
                    { label: this.translate.instant('SocialCaseReportsResourceName.entityId'), value: this.searchParams.entityName },
                    { label: this.translate.instant('SocialCaseReportsResourceName.collectorName'), value: this.searchParams.caseName },
                    { label: this.translate.instant('SocialCaseReportsResourceName.type'), value: this.searchParams.idNumber },
