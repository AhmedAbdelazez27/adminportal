  ...
  private loadMainApplyServiceData(): void {
    ...
    const subscription = this.mainApplyServiceService.getDetailById({ id }).subscribe({
      next: (response) => {
        ...
        this.fastingTentService = response.fastingTentService ?? null;
    ...
    try {
      ...
      try {
        ...
        const marker = new google.maps.Marker({
          ...
          map: this.map,
          title: this.fastingTentService.location.locationName || this.translate.instant('FASTING_TENT.SELECTED_LOCATION')
        });

        // Add info window
        const infoWindow = new google.maps.InfoWindow({
          content: this.fastingTentService.location.locationName || this.translate.instant('FASTING_TENT.SELECTED_LOCATION')
        });
