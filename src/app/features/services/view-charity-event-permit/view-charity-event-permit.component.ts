...
import { AttachmentsConfigDto, UpdateStatusDto, mainApplyServiceDto } from '../../../core/dtos/mainApplyService/mainApplyService.dto';
...
import { CharityEventPermitRequestService } from '../../../core/services/mainApplyService/charity-event-permit-request.service';
...
@Component({
  selector: 'app-view-charity-event-permit',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, TranslateModule, RouterLink, NgSelectModule],
  templateUrl: './view-charity-event-permit.component.html',
  styleUrl: './view-charity-event-permit.component.scss'
})
export class ViewCharityEventPermitComponent implements OnInit, OnDestroy {
  ...
  loadAttachmentConfigs(type: AttachmentsConfigType): void {
    ...
    s.sub = this.attachmentService.getAttachmentsConfigByServiceType(type).subscribe({
      next: (configs: any) => {
        s.configs = configs || [];
  ...
  submitRejectComment(): Observable<any> {
    ...
    const params: Partial<WorkFlowCommentDtos> = {
      id: null,
      empId: localStorage.getItem('userId'),
      workFlowStepsId: this.originalworkFlowId,
      comment: "ملاحظات: " + this.originalNotes,
